...
// Function prototypes
void ExpiredProductsByCertainDate(Product products);
void ExpiredProductsByCertainDate(ExpiryDate date, Product products);
void ProductsLessThan(int quantity, Product products);
int GetIndexOfHighestSales(Product products);
...
int main() {
    cin >> n_products;
    Product *products = new Product[n_products];
    ...
    for (int i = 0; i < n_products; i++) {
        ...
        cout << "The Price: ", cin >> products[i].price;
    }

    // Part 1: Display products that will expire at 1/1/2020.​
    ExpiredProductsByCertainDate(products);
    ...
    ExpiredProductsByCertainDate(userInput, products);
    ...
    ProductsLessThan(quantity, products);
    ...
    cout << products[GetIndexOfHighestSales(products)].name << endl;
}
